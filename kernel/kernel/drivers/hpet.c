...
void init_hpet() {
    ...
    void* hpet_addr = (void*)(hpet->addr & 0xFFFFFFFF);
    ...
    printf("CLK_PER: %x | VEN_ID: %i | LEG_CAP: %i | CNT_CAP: %i | NUM_TIM: %i | REV_ID: %i\n", clk_per, ven_id, leg_cap, cnt_cap, num_tim, rev_id);
    *gen_conf &= ~((uint64)0b11);
    *main_cnt = 0;
    for(uint8_t i = 0; i < num_tim; i++) {
        printf("---- TIMER %i ----", i)
        volatile uint64_t* tim_n_conf = (volatile uint64_t*)(tim_n_conf_base + 0x20 * i);
